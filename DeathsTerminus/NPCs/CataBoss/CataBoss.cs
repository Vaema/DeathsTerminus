...
namespace DeathsTerminus.NPCs.CataBoss
{
    ...
    public class CataBoss : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            switch (npc.ai[0])
            {
                ...
                /*case 27:
                    //? secs each
                    //MothsAndMothronsAndLampAttack();
                case 28:
                    //? secs each
                    //SomeSortOfUltimateScythesAttack();
                    break;
                case 29:
                    //7 secs each
                    //Phase3To4Animation();
        ...
        private void SpawnAnimation()
        {
            ...
            {
                //CombatText.NewText(npc.getRect(), Color.Purple, "But it will only get harder from here.", true);
            }
        ...
        private void SideScythesAttack()
        {
            ...
            if (npc.ai[1] >= 60 && npc.ai[1] % 10 == 0 && Main.netMode != 1)
            {
                ...
                for (int i = 0; i < numShots; i++)
                {
                    ...
                    Projectile.NewProjectile(launchPoint, (targetPoint - launchPoint).SafeNormalize(Vector2.Zero) * 30f, ProjectileType<CataBossScythe>(), 80, 0f, Main.myPlayer, ai0: 120f);
        ...
        private void IceSpiralAttack()
        {
            ...
            else
            {
                ...
                {
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<IceShardArena>(), 80, 0f, Main.myPlayer, ai0: 1);
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<IceShardArena>(), 80, 0f, Main.myPlayer, ai0: -1);
                }
            }
        ...
        private void SideScythesAttackHard()
        {
            ...
            if (npc.ai[1] >= 60 && npc.ai[1] < 440 && npc.ai[1] % 5 == 0 && Main.netMode != 1)
            {
                ...
                for (int i = 0; i < numShots; i++)
                {
                    ...
                    Projectile.NewProjectile(npc.Center, new Vector2(1, 0).RotatedBy(i * MathHelper.TwoPi / numShots + rotationValue) * 0.5f, ProjectileType<CataBossScythe>(), 80, 0f, Main.myPlayer, ai1: 1);
                    ...
                    if (npc.ai[1] % 10 == 0)
                    {
                        ...
                        Projectile.NewProjectile(launchPoint, (targetPoint - launchPoint).SafeNormalize(Vector2.Zero) * 30f, ProjectileType<CataBossScythe>(), 80, 0f, Main.myPlayer, ai0: 120f, ai1: 1);
        ...
        private void ShieldBonkHard()
        {
            ...
            else
            {
                ...
                {
                    Projectile.NewProjectile(player.Center + new Vector2(-npc.spriteDirection * 2048 + player.velocity.X * 64f, player.velocity.Y * 64f), new Vector2(npc.spriteDirection * 32, 0), ProjectileType<MothProjectile>(), 80, 0f, Main.myPlayer);
                }
            }
                ...
                else if (onSlimeMount)
                {
                    if (npc.ai[1] == 599)
                    {
                        ...
                        for (int i = 0; i < Main.maxProjectiles; i++)
                        {
                            ...
                            else if (Main.projectile[i].active && Main.projectile[i].type == ProjectileType<FloatingBubble>() && Main.projectile[i].ai[1] < 0)
                            {
        ...
        private void AncientDoomMinefield()
        {
            ...
            else if (((int)npc.ai[1] - 60) / period <= numRings)
            {
                ...
                if (npc.ai[1] == 60)
                {
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBossFireballRing>(), 80, 0f, Main.myPlayer, ai0: 1200, ai1: 1);
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBossFireballRing>(), 80, 0f, Main.myPlayer, ai0: 1400, ai1: -1);
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBossFireballRing>(), 80, 0f, Main.myPlayer, ai0: 1600, ai1: 1);
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBossFireballRing>(), 80, 0f, Main.myPlayer, ai0: 1800, ai1: -1);
                }
                ...
                if ((npc.ai[1] - 60) % period == 0 && Main.netMode != 1)
                {
                    ...
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBossMine>(), 80, 0f, Main.myPlayer, ai0: 0, ai1: i);
                }
            ...
            else
            {
                ...
                {
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBusterSword>(), 80, 0f, Main.myPlayer, ai0: npc.whoAmI, ai1: 1f);
                }
            }
        ...
        private void IceScythesAttack()
        {
            ...
            else
            {
                ...
                {
                    Projectile.NewProjectile(npc.Center, npc.DirectionTo(player.Center) * 0.5f, ProjectileType<CataBossSuperScythe>(), 80, 0f, Main.myPlayer, ai1: 2);
                    Projectile.NewProjectile(npc.Center, npc.DirectionTo(player.Center).RotatedBy(MathHelper.PiOver2) * 0.5f, ProjectileType<CataBossSuperScythe>(), 80, 0f, Main.myPlayer, ai1: 2);
                    Projectile.NewProjectile(npc.Center, npc.DirectionTo(player.Center).RotatedBy(-MathHelper.PiOver2) * 0.5f, ProjectileType<CataBossSuperScythe>(), 80, 0f, Main.myPlayer, ai1: 2);
                }
        ...
        private void AncientDoomMinefieldHard()
        {
            ...
            else if (((int)npc.ai[1] - 60) / period <= numRings)
            {
                ...
                if ((npc.ai[1] - 60) % period == 0 && Main.netMode != 1)
                {
                    ...
                    //ai0 is radius multiplier, ai1 is rotation
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBossMine2>(), 80, 0f, Main.myPlayer, ai0: 0, ai1: i);
                }
            ...
            else
            {
                ...
                if (npc.ai[1] == (1 + numRings) * period + 60)
                {
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<CataBusterSword>(), 80, 0f, Main.myPlayer, ai0: npc.whoAmI, ai1: 2f);
                }
            }
        ...
        private void FishronsMothsAttack()
        {
            ...
            else
            {
                ...
                else if (npc.ai[1] % 120 == 60)
                {
                    ...
                    npc.position = player.Center + new Vector2(-direction * 480, -3000);
                    npc.velocity = Vector2.Zero;

                    if (Main.netMode != 1)
                    {
                        Projectile.NewProjectile(player.Center + new Vector2(direction * 2400, 0), new Vector2(-direction * 32, 0), ProjectileType<DoomedFishron>(), 80, 0f, Main.myPlayer);
                        for (int i = 1; i <= 50; i++)
                        {
                            float yOffset = (float)(Math.Sqrt(1.5f * i - 0.5f)) * 256;
                            Projectile.NewProjectile(player.Center + new Vector2(direction * 2400 - 0.5f * direction * Math.Abs(yOffset), yOffset), new Vector2(-direction * 32, 0), ProjectileType<MothProjectile>(), 80, 0f, Main.myPlayer);
                            Projectile.NewProjectile(player.Center + new Vector2(direction * 2400 - 0.5f * direction * Math.Abs(yOffset), -yOffset), new Vector2(-direction * 32, 0), ProjectileType<MothProjectile>(), 80, 0f, Main.myPlayer);
                        }
                    }
                }
        ...
        private void MegaSprocketVsMegaBaddySuperCinematicDesperationAttack()
        {
            ...
            if (npc.ai[1] == 60 && Main.netMode != 1)
            {
                Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<MegaBaddy>(), 80, 0f, Main.myPlayer, ai0: player.whoAmI);
            }
            else if (npc.ai[1] == 120 && Main.netMode != 1)
            {
                Projectile.NewProjectile(npc.Center, Vector2.Zero, ProjectileType<MegaSprocket>(), 80, 0f, Main.myPlayer, ai0: npc.whoAmI, ai1: 2f);
            }
            ...
            for (int i = trailLength - 1; i >= 0; i--)
            {
                if (i == 0 || i % 2 == (int)npc.ai[1] % 2)
                {
                    ...
                    if (drawSpawnTransitionRing)
                    {
                        ...
                        for (int j = -1; j < 3; j++)
                        {
                            spriteBatch.Draw(ringTexture, center - Main.screenPosition, frame, spawnTransitionColor * alpha * alphaModifier * (float)Math.Pow(0.5, j), rotation, frame.Size() / 2f, scaleModifier * (float)Math.Pow(2, j), effects, 0f);
                            spriteBatch.Draw(ringTexture2, center - Main.screenPosition, frame2, spawnTransitionColor * alpha * alphaModifier * (float)Math.Pow(0.5, j - 0.5), -rotation, frame2.Size() / 2f, scaleModifier * (float)Math.Pow(2, j), effects, 0f);
                        }
                        ...
                        for (int j = 1; j <= 6; j++)
                        {
                            for (int k = 0; k < 4; k++)
                            {
                                ...
                                spriteBatch.Draw(silhouetteTexture, center - Main.screenPosition + silhouetteOffset + individualOffset, npc.frame, spawnTransitionColor * alpha * alphaModifier * ((7 - j) / 6f) * 0.5f, 0f, npc.frame.Size() / 2f, 1f, effects, 0f);
                            }
                        }
                    }
                    ...
                    if (drawAura)
                    {
                        ...
                        for (int k = 0; k < 8; k++)
                        {
                            Vector2 individualOffset = new Vector2(8 + 2 * (float)Math.Sin(auraCounter / 10f), 0).RotatedBy(k * MathHelper.TwoPi / 8 + auraCounter / 20f);

                            spriteBatch.Draw(silhouetteTexture, center - Main.screenPosition + silhouetteOffset + individualOffset, npc.frame, spawnTransitionColor * (alpha * 0.5f * ((3 - (float)Math.Sin(auraCounter / 10f)) / 4f)), 0f, npc.frame.Size() / 2f, 1f, effects, 0f);
                        }
                    }
    ...
    public class CataBossScythe : ModProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            else if (projectile.ai[1] == 2)
            {
                ...
                spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.LightBlue * 0.25f, projectile.velocity.ToRotation(), new Vector2(0, 8), new Vector2(projectile.velocity.Length(), projectile.width / 16f), SpriteEffects.None, 0f);
                spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.LightBlue * 0.25f, (-projectile.velocity).ToRotation(), new Vector2(0, 8), new Vector2(projectile.width / 128f, projectile.width / 16f), SpriteEffects.None, 0f);
            }
    ...
    public class CataBossSuperScythe : ModProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            else if (projectile.ai[1] == 2)
            {
                ...
                spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.LightBlue * 0.25f, projectile.velocity.ToRotation(), new Vector2(0, 8), new Vector2(30, projectile.width / 16f), SpriteEffects.None, 0f);
                spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.LightBlue * 0.25f, (-projectile.velocity).ToRotation(), new Vector2(0, 8), new Vector2(projectile.width / 128f, projectile.width / 16f), SpriteEffects.None, 0f);
            }
    ...
    public class FishronPlatform : ModProjectile
    {
            ...
            if (projectile.ai[1] == 0)
            {
                ...
                if (bubbleShotProgress >= 80)
                {
                    ...
                    if (bubbleCount == 4)
                    {
                        bubbleCount = 0;
                        if (Main.netMode != 1)
                            Projectile.NewProjectile(projectile.Center, Vector2.Zero, ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 0.995f, ai1: 0.1f);
                    }
                    else
                    {
                        if (Main.netMode != 1)
                            Projectile.NewProjectile(projectile.Center, Vector2.Zero, ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 0.95f, ai1: -0.08f);
                    }
                }
            }
            else
        ...
        public override void Kill(int timeLeft)
        {
            if (timeLeft > 0)
            {
                ...
                for (int i=0; i<12; i++)
                {
                    Projectile.NewProjectile(projectile.Center, new Vector2(boss.velocity.X * 0.5f, 0) + new Vector2(6, 0).RotatedBy(i * MathHelper.TwoPi / 8), ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 0.99f, ai1: 0.08f);
                    Projectile.NewProjectile(projectile.Center, new Vector2(boss.velocity.X * 0.5f, 0) + new Vector2(3, 0).RotatedBy((i + 0.5f) * MathHelper.TwoPi / 8), ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 0.99f, ai1: 0.08f);
                }
            }
        }
    ...
    public class DoomedFishron : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (bubbleShotProgress >= bubbleShotProgressRequired)
            {
                ...
                if (Main.netMode != 1)
                {
                    Projectile.NewProjectile(projectile.Center - new Vector2(projectile.direction * bubbleShotProgress, 0), Vector2.Zero, ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 1f, ai1: 0.05f);
                    Projectile.NewProjectile(projectile.Center - new Vector2(projectile.direction * bubbleShotProgress, 0), Vector2.Zero, ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 1f, ai1: -0.05f);
                }
            }
            ...
            if (projectile.timeLeft == 60)
            {
                if (Main.netMode != 1)
                {
                    for (int i = 0; i < 12; i++)
                    {
                        Projectile.NewProjectile(projectile.Center, new Vector2(6, 0).RotatedBy(i * MathHelper.TwoPi / 8), ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 1f);//, ai0: 0.99f, ai1: 0.08f);
                        Projectile.NewProjectile(projectile.Center, new Vector2(3, 0).RotatedBy((i + 0.5f) * MathHelper.TwoPi / 8), ProjectileType<FloatingBubble>(), 80, 0f, Main.myPlayer, ai0: 1f);//, ai0: 0.99f, ai1: 0.08f);
                    }
                }
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.Teal * 0.25f, projectile.velocity.ToRotation(), new Vector2(0, 8), new Vector2(projectile.velocity.Length(), projectile.width / 16f), SpriteEffects.None, 0f);
            spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.Teal * 0.25f, (-projectile.velocity).ToRotation(), new Vector2(0, 8), new Vector2(projectile.width / 128f, projectile.width / 16f), SpriteEffects.None, 0f);
    ...
    public class MothProjectile : ModProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.DarkOliveGreen * 0.25f, projectile.velocity.ToRotation(), new Vector2(0, 8), new Vector2(projectile.velocity.Length(), projectile.width / 16f), SpriteEffects.None, 0f);
            spriteBatch.Draw(mod.GetTexture("NPCs/CataBoss/CataBossScytheTelegraph"), projectile.Center - Main.screenPosition, new Rectangle(0, 0, 64, 16), Color.DarkOliveGreen * 0.25f, (-projectile.velocity).ToRotation(), new Vector2(0, 8), new Vector2(projectile.width / 128f, projectile.width / 16f), SpriteEffects.None, 0f);
    ...
    public class MegaSprocket : ModProjectile
    {
        ...
        public override void AI()
        {
            projectile.scale = projectile.ai[1] + 1f;
            Vector2 oldCenter = projectile.Center;
            projectile.width = (int)(projectile.scale * 18);
            projectile.height = (int)(projectile.scale * 18);
            projectile.Center = oldCenter;
            ...
            if (projectile.timeLeft == lifeTime)
            {
                ...
                {
                    if (Main.netMode != 1)
                        Projectile.NewProjectile(projectile.Center, new Vector2(1, 0).RotatedBy(i * MathHelper.TwoPi / 6f), ProjectileType<MegaSprocketPrism>(), 80, 0f, Main.myPlayer, ai0: i, ai1: projectile.whoAmI);
                }
            }
    ...
    public class MegaSprocketPrism : ModProjectile
    {
        ...
        {
            float point = 0f;
            return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), projectile.Center + new Vector2(20, 0), projectile.Center + new Vector2(4096, 0).RotatedBy(projectile.rotation), 44 * projectile.scale, ref point);
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (firingPrism)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    Texture2D tex7 = ModContent.GetTexture("Terraria/Projectile_632");
                    //laser length
                    float num528 = 1200 - (48 + Math.Max(0, (projectile.localAI[0] - 120))) - 64;
                    num528 = num528 * (1 + 30f / Math.Max(1, projectile.timeLeft - 60));

                    //Prisms here are different, each one is a different color
    ...
    public class MegaBaddy : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            projectile.ignoreWater = true;
            projectile.scale = 2f;
            projectile.timeLeft = initTime + lifeTime + fadeTime;
        }

        public override void AI()
        {
            if (projectile.timeLeft > lifeTime + fadeTime)
            {
                projectile.scale -= 1 / 60f;
            }
            else if (projectile.timeLeft > fadeTime)
            {
                float increment = 1 / 16f + (lifeTime - (projectile.timeLeft - fadeTime)) / (float)lifeTime / 6f;
                ...
                if (projectile.localAI[0] >= 1 && Main.netMode != 1)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        float shotRotation = (lifeTime - (projectile.timeLeft - fadeTime) - shotExtraTime) * (lifeTime - (projectile.timeLeft - fadeTime) - shotExtraTime) / 50000f + i * MathHelper.TwoPi / 6f;
                        Projectile.NewProjectile(projectile.Center + new Vector2(projectileRadius, 0).RotatedBy(shotRotation) + new Vector2(-4, 0).RotatedBy(shotRotation) * shotExtraTime, new Vector2(-4, 0).RotatedBy(shotRotation), ProjectileType<Shadow>(), 80, 0f, Main.myPlayer);
                    }
                }
            }
            else
            ...
            {
                projectile.alpha = (int)(255 - (projectile.timeLeft / 690f) * 255);
            }
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            return new Vector2(circleCenter.X - furthestX, circleCenter.Y - furthestY).Length() > projectileRadius;
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int i=0; i < numDraws; i++)
            {
                ...
                float distanceInwards = (i * i + (i % 3 + 1) * (lifeTime - projectile.timeLeft)) % 60;
                float rotationOffset = (i * i) % numDraws + (i * i + (i % 3 + 1) * (lifeTime - projectile.timeLeft)) / 60;
                float alpha = (60 - distanceInwards) / 60;
                spriteBatch.Draw(texture, projectile.Center + new Vector2(projectileRadius - distanceInwards, 0).RotatedBy(i * MathHelper.TwoPi / numDraws + rotationOffset) - Main.screenPosition, frame, Color.White * alpha * alphaModifier, 0f, frame.Size() / 2, projectile.scale, effects, 0f);
            }
            ...
            for (int i = 0; i < numDraws; i++)
            {
                float distanceInwards = (i * i + 3 * (lifeTime - projectile.timeLeft)) % 45;
                float rotationOffset = (i * i) % numDraws + (i * i + 2 * (lifeTime - projectile.timeLeft)) / 45;
                float alpha = (45 - distanceInwards) / 45;
                spriteBatch.Draw(texture, projectile.Center + new Vector2(projectileRadius - distanceInwards, 0).RotatedBy(i * MathHelper.TwoPi / numDraws + rotationOffset) - Main.screenPosition, frame, Color.White * alpha * alphaModifier, 0f, frame.Size() / 2, projectile.scale, effects, 0f);
            }
            ...
            for (int i = 0; i < numDraws; i++)
            {
                spriteBatch.Draw(texture, projectile.Center + new Vector2(projectileRadius, 0).RotatedBy(i * MathHelper.TwoPi / numDraws) - Main.screenPosition, frame, Color.White * alphaModifier, i * MathHelper.TwoPi / 48, new Vector2(0, 0.5f), projectile.scale * projectileRadius, effects, 0f);
            }

            //draw the baddy's eyes
            texture = mod.GetTexture("NPCs/CataBoss/BaddyPet_Eyes");
            frame = texture.Frame(1, 3, 0, 0);
            effects = projectile.spriteDirection == 1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally;
            spriteBatch.Draw(texture, projectile.Center + new Vector2(projectileRadius + 240, 0).RotatedBy(projectile.rotation) - Main.screenPosition, frame, Color.White * alphaModifier, 0f, frame.Size() / 2, projectile.scale * 2f, effects, 0f);

            if (projectile.timeLeft > fadeTime)
            {
                for (int i = 0; i < 6; i++)
                {
                    float rotation = (lifeTime - (projectile.timeLeft - fadeTime)) * (lifeTime - (projectile.timeLeft - fadeTime)) / 50000f + i * MathHelper.TwoPi / 6f;
                    effects = rotation > MathHelper.Pi ? SpriteEffects.None : SpriteEffects.FlipHorizontally;
                    spriteBatch.Draw(texture, projectile.Center + new Vector2(projectileRadius + 120, 0).RotatedBy(rotation) - Main.screenPosition, frame, Color.White * alphaModifier, 0f, frame.Size() / 2, projectile.scale, effects, 0f);
                }
            }
